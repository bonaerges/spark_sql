...
print(f"defaultParallelism: {sessionSpark.sparkContext.defaultParallelism}")
...
## lista que almacena los hilos para hacer un control completo
threads=[]
## Función con la acción de escritura. Esta función se ejecutará en hilos paralelos
def writeTable(vTabla=str, df=DataFrame):
    df.write.mode("overwrite").saveAsTable("default.{}".format(vTabla.lower()))
    return None
